...
BEGIN   {
        push (@INC, "/ufrc/soltis/shan158538/TragFL_NewAnalysis/OutPut/wublast/parser_single_hit/");
}
	...
	# print "$qname $qlength\n";
	 ...
	 SUBJECT:while(my $subject = $blast->nextSbjct) {
		 ...
		 while(my $hsp = $subject->nextHSP) {
			...
			#next unless ($id >= 90) && ($pval <= 1e-100) && ((abs($se-$sb)/$slength > 0.9) || (abs($qe-$qb)/$qlength > 0.9));# && ($alig_test > 0.9); # changed according to 2018 paper
			#my $xyFLAG = XY($qlength, $slength, $qb, $qe, $sb, $se, $strand, $allowed_overhang); 
			#next unless ($qlength >= 1000) && ($slength >= 1000);# && ($xyFLAG == 0); #($xyFLAG == 0); #&& ($qlength <= 5000) && ($slength<=5000);
